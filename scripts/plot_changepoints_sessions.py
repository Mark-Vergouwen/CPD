...
def plot_changepoints_sessions(aggregated_breakpoints, contrib_features_dict, features_df, sessions):
    """
    Plot time series data for all contributing features and overlay aggregated changepoints and charging sessions.
    ...
    - sessions: List of tuples representing charging sessions, each containing a start and end timestamp.
    
    OUTPUT:
    - Time series plot with all contributing features, overlaid aggregated changepoints, and shaded charging sessions
    """
    ...
    # Shade areas for charging sessions
    for session in sessions:
        start_time, end_time = session
        plt.axvspan(start_time, end_time, color='orange', alpha=0.3, label="Charging Session" if start_time == sessions[0][0] else "")
    
    # Plot formatting
    plt.xlabel("Datetime", fontsize=14)
    plt.ylabel("Feature Value", fontsize=14)
    plt.title("Aggregated Breakpoints with Contributing Features and Charging Sessions", fontsize=16)
